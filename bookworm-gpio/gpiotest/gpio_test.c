...
#define LED_OFFSET  24   /* global 536 */
#define SW_OFFSET   23   /* global 535 */
...
void pwm_open( void )
{
    ...
    bool isdir;
    char path[100] = "";
    strcpy( path, "/sys/class/pwm/pwmchip0/pwm1" );
    struct stat st = {0};
    if ( !stat(path, &st) ) {
...
{
    FILE *fp;
    fp = fopen( "/sys/class/pwm/pwmchip0/pwm1/polarity", "w" );
    fputs( "normal", fp );
    fclose( fp );
}
...
{
    FILE *fp;
    fp = fopen( "/sys/class/pwm/pwmchip0/pwm1/enable", "w" );
    fprintf( fp, "%d", 1 );
    fclose( fp );
}
...
int pwm_freq( int freq )
{
    ...
    fp = fopen( "/sys/class/pwm/pwmchip0/pwm1/period", "w" );
...
{
    const char *spi_dev = "/dev/spidev0.0"; /* adjust if needed */
